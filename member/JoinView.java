	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == saveButton) {
			...
			for (int i = 0; i < 5; i++) {
				...
				members[i].setSsn(passwords[i]);
				members[i].setAddres(addrs[i]);
				members[i].setSsn(ssns[i]);
				serviceImp.add(members[i]);
			}

		} else if (e.getSource() == listButton) {
			Member[] members = serviceImp.getMemebers();
			for (int i = 0; i < members.length; i++) {
				System.out.println("화면으로 들어온 결과");
				System.out.println(members[i]);
				textFields[5].setText(members[i].getName()+members[i].getUserid()+members[i].getPassword()+members[i].getAddres());
			}
